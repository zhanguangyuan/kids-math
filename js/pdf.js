...
var main = {
  ...
  html2Canvas: function () {
    let shareContent = document.getElementById("mycontent");// 需要绘制的部分的 (原生）dom 对象 ，注意容器的宽度不要使用百分比，使用固定宽度，避免缩放问题
    if(shareContent === null) {
      shareContent = document.getElementById("print_id");
    }
    var width = shareContent.offsetWidth;  // 获取(原生）dom 宽度
    var height = shareContent.offsetHeight + 120; // 获取(原生）dom 高
    var offsetTop = shareContent.offsetTop + 60;  //元素距离顶部的偏移量

     //console.log("---html2Canvas---" + width +" , " +  height + ", " + offsetTop);

    var canvas = document.createElement('canvas');  //创建canvas 对象
    var context = canvas.getContext('2d');
    var scaleBy = main.getPixelRatio(context);  //获取像素密度的方法 (也可以采用自定义缩放比例)main.getPixelRatio(context)
