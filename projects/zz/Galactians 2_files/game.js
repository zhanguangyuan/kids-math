...
function createSprites() {
  try {
    ...
    g.titleback = new Image();
    g.titleback.src = g.imageData[1].src;
    g.gameback = new Image();
    g.gameback.src = "library/gamebackdrop.gif";
    g.playpause = new Image();
    g.playpause.src = "library/playpause.png";
    g.splash = new Image();
...
function wipe(doentities) {
  ...
  for (var a = 0; a < m.bonusitems.length; a++) {
    if (m.bonusitems[a].row > 0 && m.bonusitems[a].row < 3) kill(m.bonusitems[a]);
  }
...
function movePlayer(o) {
  if (!o.visible) return;
  try {
    if (m.player.moveleft) {
      o.x -= o.speed;
    }
    if (m.player.moveright) {
      o.x += o.speed;
    }
    if (o.x > (g.canvaswidth - o.w)) m.player.x = g.canvaswidth - o.w;
    if (o.x < 0) o.x = 0;
...
function endLevel() {
  ...
  g.starfighterrank = "";
  if (g.hitratio > 15) {
    g.starfighterbonus = 5000;
    g.starfighterrank = textdata[11];
  }
  if (g.hitratio > 25) {
    g.starfighterbonus = 10000;
    g.starfighterrank = textdata[12];
  }
  if (g.hitratio > 35) {
    g.starfighterbonus = 25000;
    g.starfighterrank = textdata[13];
  }
  if (g.hitratio > 45) {
    g.starfighterbonus = 50000;
    g.starfighterrank = textdata[14];
  }
